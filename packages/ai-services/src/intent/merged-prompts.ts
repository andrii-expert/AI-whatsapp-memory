...
export function buildMergedWhatsappPrompt(
  ...
  return [
    ...
    "",
    "Priority order: shopping list > task > note > reminder > event > document > address",
    ...
    "",
    "NORMAL: General conversation, greetings, questions, or messages that don't relate to tasks, notes, reminders, events, or documents",
    ...
    "    • \"what's the weather\" → Title: Normal\nI don't have access to weather information, but I can help you manage your tasks, notes, reminders, events, and documents!",
    ...
    "  Shopping list is a SPECIAL type of task. When user mentions shopping or groceries,",
    "  use the special shopping item format.",
    "",
    ...
    '    • "show my shopping list" → List tasks: Shopping List - status: all',
    '    • "what\'s on my shopping list" → List tasks: Shopping List - status: all',
    '    • "show groceries folder" → List tasks: Groceries - status: all',
    '    • "show completed items in groceries" → List tasks: Groceries - status: completed',
    ...
    '    • "mark eggs as done in groceries" → Complete a task: Eggs - on folder: Groceries',
    '    • "remove eggs from shopping list" → Delete a task: Eggs - on folder: Shopping List',
    '    • "delete milk from groceries" → Delete a task: Milk - on folder: Groceries',
    '    • "change milk to 2% milk" → Edit a task: Milk - to: 2% Milk - on folder: Shopping List',
    '    • "edit bread to whole wheat bread in groceries" → Edit a task: Bread - to: Whole Wheat Bread - on folder: Groceries',
    ...
    '    • "share groceries folder with john@example.com" → Share a shopping list folder: Groceries - with: john@example.com',
    '    • "share groceries with +27123456789" → Share a shopping list folder: Groceries - with: +27123456789',
    '    • "share groceries with John Doe" → Share a shopping list folder: Groceries - with: John Doe',
    '    • "create fruits folder in groceries" → Create a shopping list sub-folder: Groceries - name: Fruits',
    ...
    "TASK OPERATIONS:",
    "  Title: task",
    "  Create a shopping item: {item_name}  (for shopping/grocery items - folder optional)",
    ...
    "  Share a shopping list folder: {folder_route} - with: {recipient}",
    "  Create a shopping list sub-folder: {parent_folder_route} - name: {subfolder_name}",
    "  List shopping list folders: {all|parent_folder_route}",
    "    • If parent folder specified: List shopping list folders: {parent_folder_route} (shows subfolders)",
